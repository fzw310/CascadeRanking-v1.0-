...
void nonMaxSup(double **matchCost1f, int row, int col, ValStructVec<float, PointXY> &matchCost, int NSS, int maxPoint, double Rate, unsigned char step)
{
	...
	//add edge
	for (i = 1; i < row+2*step; i++)
		for(j = 1; j <col+2*step; j++)
		{
			matchCost1fMean[i][j] = 0;
		}

	for (i = step; i < row+step; i++)
		for(j = step; j <col+step; j++)
		{
			matchCost1fMean[i][j] = matchCost1f[i-step][j-step];
		}
	...
	for (i = 0; i < valPnt.size(); i++){
		...
		{
			free(isMax1u);
            for(int kk = 0; kk < row+2*step;kk++)
                free(matchCost1fMean[kk]);
